...
# 정규 시즌과 포스트 시즌 데이터를 수집하여 MongoDB에 저장
for year in range(2017, 2025):
    for month in range(3, 10):  # 정규 시즌 (3월부터 10월까지)
        print(f"{year}년 {month}월 정규 시즌 데이터를 수집합니다.")
        schedule_data = crawl_kbo_schedule(str(year), f"{month:02d}", "regular")
        save_to_mongodb(schedule_data)

    for month in range(8, 12):  # 포스트 시즌 (8월부터 11월까지)
        print(f"{year}년 {month}월 포스트 시즌 데이터를 수집합니다.")
        schedule_data = crawl_kbo_schedule(str(year), f"{month:02d}", "postseason")
        save_to_mongodb(schedule_data)
